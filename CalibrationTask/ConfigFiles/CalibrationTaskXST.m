...
function string = calibpoint(list)
    ...
    X = X/conversion;
    Y = Y/conversion;
    fix.xCenter = X; %changing target params
    fix.yCenter = Y;
    fix.colors = Calib.fgcolor;
    fix.isVisible = true;
    
    text.x = X; %changing text params
    text.y = Y;
    text.isVisible = true;
    ...
    %Drawing point
    if strcmp(string, 'Early')
        changeT.onsetTime = -1;
    elseif strcmp(string, 'Feedback')
        changeT = dotsDrawable.drawFrame({fix}); %Draw the changed dot
        
        %Wait for response and compute RT
         tic   
         while toc < rtwindow
             read(ui);
             [a, b, eventname, d] = ui.getHappeningEvent();
             if ~isempty(eventname)
                 ...
                 responseT = responseT(end)-changeT.onsetTime;
                 if responseT >= 0
