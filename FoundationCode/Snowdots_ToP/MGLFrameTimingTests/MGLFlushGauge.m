    ...
    methods
        ...
        function success = initialize(self)
            ...
            if sum(self.initialFrameIsValid) >= self.initialFrameMinimum
                success = true;
                self.framePeriod = ...
                    mean(self.initialFrameIntervals(self.initialFrameIsValid));
            else
                success = false;
        ...
        function [onsetTime, onsetFrame, swapTime, isTight] = flush(self)
        ...
        function [onsetTime, onsetFrame, swapTime, isTight] = flushWait(self)
            [onsetTime, onsetFrame, swapTime, isTight] = self.flush();
            mglWaitSecs(onsetTime - mglGetSecs() + self.framePadding);
        end
        
        % Clear both frame buffers, and implicitly update reference frame.
        function [onsetTime, onsetFrame, swapTime, isTight] = blank(self)
            mglClearScreen();
            [onsetTime, onsetFrame, swapTime, isTight] = self.flush();
            mglClearScreen();
            self.flush();
